...
const POWER_MODE = 3
const TEMP_RES = 5
const PRESSURE_RES = 5
const FILTER = 4
const STANDBY = 4

// combine bits for config
const CONFIG = (STANDBY << 5) + (FILTER << 2)
// combine bits for ctrl_meas
const CTRL_MEAS = (TEMP_RES << 5) + (PRESSURE_RES << 2) + POWER_MODE

//read commands
const REGISTER_DIG_T1 = 0x88,
...
console.log('sensor', enviro.readByteSync(ADDR, REGISTER_CHIPID), 0x58)

enviro.readI2cBlockSync(ADDR, REGISTER_DIG_T1, 2, buff)
const DIG_T1 = buff.readUInt16LE()
enviro.readI2cBlockSync(ADDR, REGISTER_DIG_T2, 2, buff)
const DIG_T2 = buff.readInt16LE()
enviro.readI2cBlockSync(ADDR, REGISTER_DIG_T1, 2, buff)
const DIG_T3 = buff.readInt16LE()
...
function readTemp(cb) {
    async.parallel([
        function (cb) {
            enviro.readByte(ADDR, REGISTER_TEMPDATA_MSB, cb)
        },
        function (cb) {
            enviro.readByte(ADDR, REGISTER_TEMPDATA_LSB, cb)
        },
        function (cb) {
            enviro.readByte(ADDR, REGISTER_TEMPDATA_XLSB, cb)
        }
    ], (err, res) => {
        if (err)
            return cb(err)
        ...
        tempVar1 = (raw / 16384.0 - DIG_T1 / 1024.0) * DIG_T2 // formula for temperature from datasheet
        tempVar2 = (raw / 131072.0 - DIG_T1 / 8192.0) * (raw / 131072.0 - DIG_T1 / 8192.0) * DIG_T3 // formula for temperature from datasheet
        temp = (tempVar1 + tempVar2) / 5120.0 // formula for temperature from datasheet
